...
| On-disk format compatibility | Ready | [Format spec](format-spec.md), [Reliability](reliability.md), [Release procedure](../internal/process/release-procedure.ja.md) | The current reader fails closed on unsupported future versions, tests protect that behavior, and the release procedure now requires migration/export notes for breaking format work. |
| Benchmark baseline | Ready | [Benchmarks](benchmarks.md), [Scale baseline](../internal/process/scale-baseline-v0.7.0.ja.md) | Reproducible baseline now covers insert, traversal, export/import, diff, key-aware merge preview, and trace payload generation with recorded 2,000-node results. |
