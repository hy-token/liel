...
def test_python_api_benchmark_output_includes_counts_and_file_size():
    ...
    assert any(line.startswith("export_json          bytes=") for line in results)
    assert any(line.startswith("import_roundtrip     records=9") for line in results)
    assert any(line.startswith("diff_roundtrip       changes=0") for line in results)
    assert any(line.startswith("merge_preview        reused=9") for line in results)
    assert any(line.startswith("trace_full_path      path_nodes=5") for line in results)
